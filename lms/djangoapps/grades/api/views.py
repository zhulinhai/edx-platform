        ...
        if isinstance(course, Response):
            ...
            if call_origin_requires_intervention(request):
                ...
                if len(possible_courses) == 1:
                    ...
                    preferred = configuration_helpers.get_value('PREFERRED_ORG_FOR_CERTIFICATES')
                    for item in possible_courses:
                        if item.org == preferred:
                            course = item

                    log.info('Multiple courses found on the intervention [%s]. Using [%s]'
                             'based on the site PREFERRED_ORG_FOR_CERTIFICATES',
                              possible_courses, unicode(course.id))


            if isinstance(course, Response):
                # Returns a 404 if course is still invalid, or request.user is not enrolled in the course
                return course
